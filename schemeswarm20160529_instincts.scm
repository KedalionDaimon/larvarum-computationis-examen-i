...
; BEGIN OF INSTINCT IMPLANT

; setup instincts - "must have" in reality
(define (word-instinct word)
...
(define (proto-apply-instincts sentence checked-sentence)
    (if (null? sentence)
...
(define main
  (let ((swarmdata (open-input-file "swarmdata.txt"))
    ...
    (begin
      ...
      (set! maininput (apply-instincts (takelastn inmemory (read swarminput)))) (close-input-port swarminput)
      (set! insistence (read swarminsistence)) (close-input-port swarminsistence)
      (set! inputhistory (apply-instincts (read swarmhistory))) (close-input-port swarmhistory)
